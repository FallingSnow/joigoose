...
internals.convert = joiObject => {
  ...
  if (joiObject._type === "alternatives") {
    joiObject._inner.matches.forEach(match => {
      if (match.then)
        match.then._inner.children.forEach(child => {
          output[child.key] = {...output[child.key], ...internals.convert(child.schema)};

          if (output[child.key].required)
            output[child.key].requiredChecks = [match.peek];
      ...
      if (match.otherwise)
        match.otherwise._inner.children.forEach(child => {
          const info = internals.convert(child.schema);
          if (
            typeof output[child.key] !== "undefined" &&
            output[child.key].type !== info.type
          ) {
            output[child.key].type = internals.mongoose.Schema.Types.Mixed;
          } else {
            output[child.key] = {...output[child.key], ...info};
          }

          if (info.required)
            output[child.key].requiredChecks = [
              ...output[child.key].requiredChecks,
              match.peek
            ];
      ...
      Object.entries(output).forEach(([key, child]) => {
        if (!child.requiredChecks) return;

        output[key].required = function() {
          for (const checker of child.requiredChecks) {
            if (!checker.validate(this).error) return true;
          }
          return false;
        };
      });
...
internals.typeDeterminer = joiObject => {
  ...
  if (joiObject._type === "alternatives") {
    ...
    for (i = 0; i < joiObject._inner.matches.length; i++) {
      const match = joiObject._inner.matches[i];
      const type = match.then ? match.then._type : match.schema._type;
      types[type] = types[type] ? types[type] + 1 : (types[type] = 1);
    }
