...
internals.convert = joiObject => {
  ...
  if (joiObject._type === "alternatives") {
    joiObject._inner.matches.forEach(child => {

      if (child.then)
      child.then._inner.children.forEach(child => {
        output[child.key] = internals.convert(child.schema);
        output[child.key].required = function() {return child.peek.validate(this)};
	...
	if (internals.isObjectId(child.schema)) {
      ...
      if (child.otherwise)
      child.otherwise._inner.children.forEach(child => {
        const info = internals.convert(child.schema);
	if (typeof output[child.key] !== 'undefined' && output[child.key].type !== info.type) {
